# STELLA — BUILD GUIDE & SMART CONTRACT BEST PRACTICES
...
pub fn init_escrow(env: Env, employer: Address, candidate: Address, amount: i128) {
    ...
    if amount <= 0 { panic_with_error!(&env, FlowError::InvalidAmount); }

    // STEP 3: Check preconditions (no existing escrow)
    if env.storage().persistent().has(&DataKey::Escrow(candidate.clone())) {
        panic_with_error!(&env, FlowError::EscrowAlreadyExists);
    }
...
panic_with_error!(&env, FlowError::EscrowNotFound);

// ✅ Also fine — Result<> return for cleaner V1 code
pub fn clawback(env: Env, employer: Address) -> Result<i128, FlowError> {
...
mod test {
    ...
    fn test_init_escrow() {
        ...
        let contract_id = env.register_contract(None, StelllaroidFlow);
        let client = StelllaroidFlowClient::new(&env, &contract_id);
...
export const connectWallet = async (): Promise<string> => {
  ...
  if (!connection.isConnected) {
    throw new Error(
      "Freighter wallet extension is not installed. Install it at freighter.app",
    );
  }
...
export const parseContractError = (error: unknown): string => {
  ...
  if (msg.includes("insufficient balance"))
    return "Insufficient XLM balance in wallet.";
...
if milestone_amount > remaining {
    panic_with_error!(&env, FlowError::ExceedsTotal);
}
...
const EMPLOYER_SECRET =
  "SCZANGBA5QLPCE2NQTNEW5C4RPI6POPFY5TVHZFBVBQNKNLCDXOQHH0";
...
let escrow = env.storage().persistent()
    ...
    .unwrap_or_else(|| panic_with_error!(&env, FlowError::EscrowNotFound));
```
...
stellar contract deploy \
  --wasm target/wasm32-unknown-unknown/release/stellaroid_flow.wasm \
  --source my-key \
  --network testnet
...
- Write `lib.rs` types first (`DataKey`, `Escrow`, `FlowError`), then functions, then tests
